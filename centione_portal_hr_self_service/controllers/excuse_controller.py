...
class CustomerPortal(CustomerPortal):
    ...
    def portal_my_excuse_request(self, page=1, sortby=None, search=None, search_in='all',**kw):
        ...
        excuses_coach = excuse_obj.sudo().search(domain_ch, order=order, limit=self._items_per_page, offset=pager['offset'])
        
        domain_mng += [
            ('employee_id.parent_id.user_id', 'in', [request.env.user.id,]),
            
        ]
        excuses_manager = excuse_obj.sudo().search(domain_mng, order=order, limit=self._items_per_page, offset=pager['offset'])
        ...
        values.update({
            ...
            'open_this1':'defaultOpen3',
            'open_this2':'defaultOpen3',
            'open_this3':'defaultOpen',
        })
